...
router.delete("/tickets/:id", requireRole(["admin", "manager"]), async (req: AuthRequest, res) => { const existing = await Ticket.findOne({ _id: req.params.id, organization: oid(req) }); if (!(await requireTicketAccess(req, res, existing)) || !existing) return; const ticket = await Ticket.findOneAndDelete({ _id: req.params.id, organization: oid(req) }); return ticket ? res.status(204).send() : res.status(404).json({ message: "Ticket not found" }); });
