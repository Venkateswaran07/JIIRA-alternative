...
const audit = (req: AuthRequest, action: string, entityType?: string, entityId?: unknown, metadata: object = {}) => AuditEvent.create({ organization: oid(req), actor: uid(req), action, entityType, entityId: entityId ? String(entityId) : undefined, metadata });

const userPatch = z.object({ name: z.string().min(2).optional(), role: z.enum(["admin", "manager", "engineer", "designer"]).optional(), skills: z.array(z.string()).optional(), availability: z.number().min(0).max(1).optional(), capacity: z.number().min(0).optional(), avatarColor: z.string().optional() });
...
router.post("/invitations", requireRole(["admin"]), async (req: AuthRequest, res) => {
  const body = parseOr400(z.object({ name: z.string().min(2), email: z.string().email(), role: z.enum(["admin", "manager", "engineer", "designer"]).default("engineer") }), req.body, res); if (!body) return;
  ...
  const token = crypto.randomBytes(32).toString("base64url"); await ActionToken.create({ user: user._id, organization: oid(req), kind: "invite", tokenHash: hash(token), expiresAt: new Date(Date.now() + 7 * 86400_000) });
...
router.post("/invitations/:userId/resend", requireRole(["admin"]), async (req: AuthRequest, res) => {
  ...
  await ActionToken.deleteMany({ user: user._id, kind: "invite", usedAt: { $exists: false } }); const token = crypto.randomBytes(32).toString("base64url"); await ActionToken.create({ user: user._id, organization: oid(req), kind: "invite", tokenHash: hash(token), expiresAt: new Date(Date.now() + 7 * 86400_000) }); return res.json({ ok: true, ...(env.nodeEnv !== "production" ? { inviteToken: token } : {}) });
});
