...
export interface IOrganization { company: string; name: string; slug: string; plan: "starter" | "scale" | "enterprise"; owner: string; onboardingCompletedAt?: Date; settings: { riskThreshold: number; sprintLengthDays: number; weeklyCapacityHours: number; timezone: string; aiEnabled: boolean; slaPolicy: Record<"critical" | "high" | "medium" | "low", { firstResponseHours: number; resolutionHours: number }> } }
...
export const Organization = createPgModel({ table: "organizations", columns: ["company", "name", "slug", "plan", "owner", "onboardingCompletedAt", "settings"], json: ["settings"], defaults: { plan: "starter", settings }, relations: { owner: { model: () => User } } });
