...
export const OutboxEvent = createPgModel({ table: "outbox_events", columns: ["organization", "eventType", "aggregateType", "aggregateId", "payload", "dedupeKey", "status", "attempts", "nextAttemptAt", "lastError", "processedAt"], json: ["payload"], defaults: { payload: {}, status: "pending", attempts: 0 } });
export const OutboxDelivery = createPgModel({ table: "outbox_deliveries", columns: ["organization", "event", "channel", "destination", "destinationRef", "payload", "status", "attempts", "nextAttemptAt", "lastError", "lastErrorAt", "processedAt"], json: ["payload"], defaults: { payload: {}, status: "pending", attempts: 0 } });
