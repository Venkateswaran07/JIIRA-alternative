...
const contracts: Record<string, Omit<AiMutationContract, "endpoint">> = {
  ...
  "POST /companies/:companyId/workspaces": { body: "{ name: string (min 2), slug?: lowercase letters, numbers, and hyphens }" },
  "POST /companies/:companyId/groups": { body: "{ name: string (min 2), description?: string }" },
  "PATCH /companies/:companyId/groups/:id": { body: "Any subset of { name?: string (min 2), description?: string }" },
  "PUT /companies/:companyId/groups/:id/members": { body: "{ userIds: string[] }" },
  "PUT /companies/:companyId/groups/:id/workspaces": { body: "{ grants: Array<{ workspace: string, role: admin|manager|engineer|designer }> }" },
